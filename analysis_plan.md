...
`outputs/reports/paper_materials/framework_v2/` 只根据已有 reviewed payload 和 artifacts 输出材料。当前 framework_v2 reviewed 补码已覆盖 `514/514` 条 `quality_v5 post-only` 正式帖子，新增 F/G/H/I/J/K 字段可作为正式 v2 描述性统计使用。

方法边界仍需保留：F/G/H/I/J/K 字段来自“用户授权接受 AI 辅助编码草稿并保留 provenance”的补码流程，可证明覆盖完整性和工程一致性，但不得写作“双人独立人工复核”或“编码完全无误”。如后续论文需要更强方法说服力，应对 `framework_v2_coding_audit_report` 中的高风险复核队列进行人工抽查并记录修正率。

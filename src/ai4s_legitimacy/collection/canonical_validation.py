...
def normalize_canonical_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    normalized["review_status"] = (
        str(normalized.get("review_status") or "unreviewed").strip() or "unreviewed"
    )
...
def require_allowed(value: str, *, allowed: Iterable[str], field_name: str) -> None:
    if value not in allowed:
        expected = ", ".join(sorted(allowed))
        raise ValueError(f"{field_name} must be one of: {expected}. Got {value!r}")


def require_evidence_if_codes(codes: list[str], evidence: list[str], *, field_name: str) -> None:
...
def validate_canonical_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    require_allowed(normalized["record_type"], allowed=RECORD_TYPE_VALUES, field_name="record_type")
    require_allowed(normalized["decision"], allowed=DECISION_VALUES, field_name="decision")
    require_allowed(
        normalized["context_available"],
        allowed=CONTEXT_AVAILABLE_VALUES,
        field_name="context_available",
    )
    require_allowed(normalized["context_used"], allowed=CONTEXT_USED_VALUES, field_name="context_used")
    require_allowed(

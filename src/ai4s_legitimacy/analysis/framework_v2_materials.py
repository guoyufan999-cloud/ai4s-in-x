...
def _write_alignment(output_dir: Path, summary: dict[str, Any]) -> Path:
    metadata = summary["metadata"]
...
- 已 reviewed 正式帖子：`{metadata["framework_v2_reviewed_posts"]}`
- 未完成正式帖子：`{metadata["framework_v2_missing_posts"]}`
- 是否完成：`{metadata["framework_v2_coding_complete"]}`
- 说明：{status_note}
"""
...
def _write_chapter_materials(output_dir: Path, summary: dict[str, Any]) -> Path:
    ...
    completion_sentence = (
        ...
        if metadata["framework_v2_coding_complete"]
        else "F/G/H/I/J/K 尚未完成全部正式 reviewed 补码；相关表只能作为阶段性材料。"
    )
...
### AI介入方式分布
{_markdown_table(tables["ai_intervention_mode_distribution"], empty_note=note)}

### AI介入强度分布
{_markdown_table(tables["ai_intervention_intensity_distribution"], empty_note=note)}
...
### 评价张力分布
{_markdown_table(tables["evaluation_tension_distribution"], empty_note=note)}

### 正式规范参照分布
{_markdown_table(tables["formal_norm_reference_distribution"], empty_note=note)}
...
### 边界协商机制分布
{_markdown_table(tables["boundary_mechanism_distribution"], empty_note=note)}

### 边界协商结果分布
{_markdown_table(tables["boundary_result_distribution"], empty_note=note)}
...
def _write_writing_memo(output_dir: Path, summary: dict[str, Any]) -> Path:
    metadata = summary["metadata"]
    note = metadata["note"]
    if metadata["framework_v2_coding_complete"]:
        fifth_chapter = (
            "- 第五章可写：AI介入方式、介入强度、规范评价标准、规范评价倾向、"
            "评价张力与正式规范参照的组合分析。支持字段：`ai_intervention_mode_codes`、"
            "`ai_intervention_intensity_codes`、`basis_codes`、`legitimacy_codes`、"
            "`evaluation_tension_codes`、`formal_norm_reference_codes`。"
        )
        sixth_chapter = (
            "- 第六章可写：边界类型、边界协商机制与边界结果的完整 v2 分析。"
            "支持字段：`boundary_codes`、`boundary_mode_codes`、"
            "`boundary_mechanism_codes`、`boundary_result_codes`。"
        )
        data_section_title = "## 完成状态"
    else:
        fifth_chapter = (
            "- 第五章可写：已有 B/C 组支持的规范评价倾向与评价标准。"
            "支持字段：`legitimacy_codes`、`basis_codes`。"
        )
        sixth_chapter = (
            "- 第六章可写：已有 D 组支持的边界类型。"
            "支持字段：`boundary_codes`、`boundary_mode_codes`。"
        )
        data_section_title = "## 数据不足提醒"
...
def generate_framework_v2_materials(
    *,
    ...
    paths = {
        ...
        "alignment": str(_write_alignment(output_dir, summary)),
        ...
        "writing_memo": str(_write_writing_memo(output_dir, summary)),
    }
    return {

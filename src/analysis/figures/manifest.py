...
FIGURE_ENTRIES = [
    {
        ...
        "title": "月度正式帖子趋势",
        "placement": "正文",
        "data_basis_key": "posts_trend",
        "takeaway_key": "posts_trend",
    },
    {
        "slug": "posts_by_period",
        "title": "半年度帖子与评论规模",
        "placement": "补充材料",
        "data_basis_key": "posts_by_period",
        "takeaway_key": "posts_by_period",
    },
    {
        "slug": "posts_by_quarter",
        "title": "季度帖子与评论规模",
        "placement": "正文",
        "data_basis_key": "posts_by_quarter",
        "takeaway_key": "posts_by_quarter",
    },
    {
        "slug": "posts_heatmap",
        "title": "时间—学科—流程高频组合热力图",
        "placement": "正文",
        "data_basis_key": "posts_heatmap",
        "takeaway_key": "posts_heatmap",
    },
    {
        "slug": "comments_attitude",
        "title": "半年度评论态度结构",
        "placement": "正文",
        "data_basis_key": "comments_attitude",
        "takeaway_key": "comments_attitude",
    },
    {
        "slug": "tools_by_period",
        "title": "半年度 AI 工具构成",
        "placement": "正文",
        "data_basis_key": "tools_by_period",
        "takeaway_key": "tools_by_period",
    },
    {
        "slug": "tools_by_quarter",
        "title": "季度 AI 工具构成",
        "placement": "补充材料",
        "data_basis_key": "tools_by_quarter",
        "takeaway_key": "tools_by_quarter",
    },
    {
        "slug": "risk_themes_by_period",
        "title": "半年度风险主题构成",
        "placement": "正文",
        "data_basis_key": "risk_themes_by_period",
        "takeaway_key": "risk_themes_by_period",
    },
    {
        "slug": "risk_themes_by_quarter",
        "title": "季度风险主题构成",
        "placement": "补充材料",
        "data_basis_key": "risk_themes_by_quarter",
        "takeaway_key": "risk_themes_by_quarter",
    },
]
...
def _render_data_basis(entry: dict[str, str], coverage_end_date: str) -> str:
    ...
    basis_by_key = {
        "posts_trend": (
            ...
            f"{month_window_text}；按月补零并绘制 3 个月滚动均值。"
        ),
        "posts_by_period": (
            "研究主库 `paper_scope_quality_v4` 正式口径；帖子与评论在半年度尺度上的规模变化；"
            f"时间顺序固定为 {halfyear_sequence_text}。"
        ),
        "posts_by_quarter": (
            "研究主库 `paper_scope_quality_v4` 正式口径；帖子与评论在季度尺度上的规模变化；"
            f"时间顺序固定为 {quarter_sequence_text}。"
        ),
        "posts_heatmap": (
    ...
    return basis_by_key[entry["data_basis_key"]]


def _render_takeaway(entry: dict[str, str], coverage_end_date: str) -> str:
    ...
    takeaway_by_key = {
        "posts_trend": (
            ...
            f"{latest_halfyear_label}达到当前样本内的最高活跃度。"
        ),
        "posts_by_period": (
        ...
        "posts_by_quarter": (
            ...
            f"{latest_quarter_label}进入显著跃升区间。"
        ),
        "posts_heatmap": (
    ...
    return takeaway_by_key[entry["takeaway_key"]]
...
def write_figure_manifest(
    ...
    for index, entry in enumerate(FIGURE_ENTRIES, start=1):
        ...
        lines.append(f"- 数据口径：{_render_data_basis(entry, coverage_end_date)}")
        lines.append(f"- 正文可用结论句：{_render_takeaway(entry, coverage_end_date)}")
        if status == "generated":

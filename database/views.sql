...
DROP VIEW IF EXISTS vw_paper_quality_v4_ai_practice_distribution;
DROP VIEW IF EXISTS vw_paper_quality_v4_workflow_ai_practice_cross;
DROP VIEW IF EXISTS vw_paper_quality_v4_legitimacy_dimension_distribution;
...
-- MEDIUM: AI practice distribution (schema placeholder — data currently NULL)
CREATE VIEW vw_paper_quality_v4_ai_practice_distribution AS
SELECT
    COALESCE(c.ai_practice_code, 'uncoded') AS ai_practice_code,
    COUNT(*) AS post_count
FROM codes c
JOIN vw_posts_paper_scope_quality_v4 p ON p.post_id = c.record_id
WHERE c.record_type = 'post'
GROUP BY COALESCE(c.ai_practice_code, 'uncoded')
ORDER BY post_count DESC, ai_practice_code;

-- MEDIUM: workflow × AI practice cross (schema placeholder)
CREATE VIEW vw_paper_quality_v4_workflow_ai_practice_cross AS
SELECT
    COALESCE(p.workflow_stage, 'uncoded') AS workflow_stage,
    COALESCE(c.ai_practice_code, 'uncoded') AS ai_practice_code,
    COUNT(*) AS post_count
FROM vw_posts_paper_scope_quality_v4 p
JOIN codes c ON c.record_id = p.post_id AND c.record_type = 'post'
GROUP BY COALESCE(p.workflow_stage, 'uncoded'), COALESCE(c.ai_practice_code, 'uncoded')
ORDER BY workflow_stage, ai_practice_code;

-- MEDIUM: legitimacy dimension distribution (schema placeholder)
CREATE VIEW vw_paper_quality_v4_legitimacy_dimension_distribution AS
SELECT
    COALESCE(c.legitimacy_code, 'uncoded') AS legitimacy_code,
    COUNT(*) AS coded_count
FROM codes c
WHERE c.record_type = 'post'
  AND c.record_id IN (SELECT post_id FROM vw_posts_paper_scope_quality_v4)
GROUP BY COALESCE(c.legitimacy_code, 'uncoded')
ORDER BY coded_count DESC, legitimacy_code;
